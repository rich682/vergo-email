...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (!canPerformAction(user.role, "forms:manage", session.user.orgActionPermissions)) {
...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const viewers = await FormDefinitionService.setViewers(id, user.organizationId, userIds, user.id)
