...
const REMINDER_FREQUENCIES = [
  { value: "daily", label: "Every business day", description: "Mon-Fri" },
  { value: "every_2_days", label: "Every 2 business days", description: "e.g., Mon, Wed, Fri" },
  { value: "every_3_days", label: "Every 3 business days", description: "e.g., Mon, Thu" },
  { value: "weekly", label: "Once a week", description: "Same day each week" },
  { value: "biweekly", label: "Every two weeks", description: "Same day, every other week" }
]
            ...
            {remindersEnabled && (
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                        ...
                        {REMINDER_FREQUENCIES.map((freq) => (
                          <SelectItem key={freq.value} value={freq.value}>
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs text-gray-400">{freq.description}</span>
                            </div>
                          </SelectItem>
                        ))}
                      </SelectContent>
                  ...
                  {(reminderFrequency === "weekly" || reminderFrequency === "biweekly") && (
                    <div>
                      ...
                      >
                        ...
                        <SelectContent>
                          {DAYS_OF_WEEK.filter(d => !["0", "6"].includes(d.value)).map((day) => (
                            <SelectItem key={day.value} value={day.value}>
                              {day.label}
                            </SelectItem>
                          ))}
                  ...
                  <div className={(reminderFrequency === "weekly" || reminderFrequency === "biweekly") ? "col-span-2" : ""}>
                ...
                <p className="text-xs text-gray-500 bg-gray-50 px-3 py-2 rounded flex items-center gap-2">
                  <svg className="w-4 h-4 text-gray-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  Reminders are only sent on business days (Monday-Friday)
                </p>
              </div>
            )}
          </div>
