...
import { ClipboardList, Clock, ExternalLink, User } from "lucide-react"
...
export function BoardFormsTab({ boardId, tasks }: BoardFormsTabProps) {
  const formTasks = useMemo(
    () => tasks.filter((t) => t.taskType === "form"),
    [tasks]
  )
  ...
  if (formTasks.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-2 text-sm text-gray-500">No form tasks in this board</p>
        <p className="text-xs text-gray-400 mt-1">
          Create a task with type &ldquo;Form&rdquo; to see it here
        </p>
      </div>
    )
  }
