...
import { BarChart3, Clock, ExternalLink, User } from "lucide-react"
...
export function BoardReportsTab({ boardId, tasks }: BoardReportsTabProps) {
  const reportTasks = useMemo(
    () => tasks.filter((t) => t.taskType === "report"),
    [tasks]
  )
  ...
  if (reportTasks.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-2 text-sm text-gray-500">No report tasks in this board</p>
        <p className="text-xs text-gray-400 mt-1">
          Create a task with type &ldquo;Report&rdquo; to see it here
        </p>
      </div>
    )
  }
