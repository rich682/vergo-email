...
import { MessageSquare, Clock, ExternalLink, User } from "lucide-react"
...
export function BoardAnalysisTab({ boardId, tasks }: BoardAnalysisTabProps) {
  const analysisTasks = useMemo(
    () => tasks.filter((t) => t.taskType === "analysis"),
    [tasks]
  )
  ...
  if (analysisTasks.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-2 text-sm text-gray-500">No analysis tasks in this board</p>
        <p className="text-xs text-gray-400 mt-1">
          Create a task with type &ldquo;Analysis&rdquo; to see it here
        </p>
      </div>
    )
  }
