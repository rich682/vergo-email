...
export function FormRequestFlow({
  ...
  // Load users when moving to recipient step
  useEffect(() => {
    if (step === "select_recipients" && users.length === 0) {
      fetchUsers()
    }
  }, [step])
  ...
  const handleSend = async () => {
    if (!selectedFormId || selectedUserIds.size === 0) {
      ...
      {step === "select_recipients" && (
        <div className="space-y-4">
          <div className="text-center">
            ...
            <p className="text-sm text-gray-500">
              Choose team members to send "{selectedForm?.name}" to
            </p>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-gray-50 rounded-lg p-4 space-y-2">
            ...
            <div className="flex items-center gap-2 text-sm text-gray-600">
              ...
              <span>{selectedUserIds.size} recipients</span>
            </div>
            {boardPeriod && (
              <div className="flex items-center gap-2 text-sm text-gray-600">
